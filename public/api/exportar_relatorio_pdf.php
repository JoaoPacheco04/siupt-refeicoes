...
$html .= "<h1>RelatÃ³rio mensal â€” {$nomeMes} {$anoSel}</h1>";
$html .= '<p style="color:#888;">Gerado em ' . date('d/m/Y H:i') . '</p>';

$html .= '<table class="resumo"><tr>
    <td><strong>' . number_format($resumo['total_vendido'], 2, ',', '.') . 'â‚¬</strong>Total vendido</td>
    <td><strong>' . $resumo['total_pedidos'] . '</strong>Pedidos pagos</td>
    <td><strong>' . $resumo['total_levantados'] . '</strong>Levantadas</td>
    <td><strong>' . $resumo['total_nao_levantados'] . '</strong>NÃ£o levantadas</td>
    <td><strong>' . number_format($precoMedio, 2, ',', '.') . 'â‚¬</strong>PreÃ§o mÃ©dio</td>
</tr></table>';

if ($percentagem !== null) {
    $sinal = $percentagem >= 0 ? '+' : '';
    $html .= '<p style="color:#666;">VariaÃ§Ã£o face ao mÃªs anterior: <strong>' . $sinal . number_format($percentagem, 1, ',', '.') . '%</strong></p>';
}

$html .= '<h2>Vendas por tipo â€” ementa</h2><table><tr><th>Tipo</th><th>Qtd</th><th>Total</th></tr>';
foreach ($pratosEmenta as $i => $t) {
    ...
    $html .= "<tr{$classe}><td>" . htmlspecialchars($t['RTP_NOME']) . '</td><td>' . $t['quantidade'] . 'x</td>'
        . '<td class="total">' . number_format((float) $t['total'], 2, ',', '.') . 'â‚¬</td></tr>';
}
$html .= '</table>';

$html .= '<h2>Vendas por tipo â€” extras</h2><table><tr><th>Tipo</th><th>Qtd</th><th>Total</th></tr>';
...
$html .= '<h2>Vendas diÃ¡rias</h2><table><tr><th>Data</th><th>Pedidos</th><th>Total</th></tr>';
foreach ($vendasDiarias as $d) {
    $html .= '<tr><td>' . date('d/m/Y', strtotime($d['RP_DATA_REFEICAO'])) . '</td><td>' . $d['total_pedidos'] . '</td>'
        . '<td class="total">' . number_format((float) $d['total_vendido'], 2, ',', '.') . 'â‚¬</td></tr>';
}
