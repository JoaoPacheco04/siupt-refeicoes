...
try {
    ...
    $escrito = file_put_contents(__DIR__ . '/monitorizar_pedidos.log', $log, FILE_APPEND);
    if ($escrito === false) {
        error_log('monitorizar_pedidos.php: falha ao escrever no ficheiro de log');
    }

    echo $log;
} catch (Throwable $e) {

    error_log('monitorizar_pedidos.php: erro na monitorização — ' . $e->getMessage());
    exit(1);
}